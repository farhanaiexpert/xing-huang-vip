...
export const userLimitsTable = pgTable("user_limits", {
  ...
  pendingAmountUsdt: numeric("pending_amount_usdt", { precision: 20, scale: 8 }).default(sql`NULL`),
  pendingEffectiveAt: timestamp("pending_effective_at", { withTimezone: true }).default(sql`NULL`),
...
export const selfExclusionsTable = pgTable("self_exclusions", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").notNull().references(() => usersTable.id, { onDelete: "cascade" }),
  durationHours: integer("duration_hours").default(sql`NULL`),
  startsAt: timestamp("starts_at", { withTimezone: true }).notNull().defaultNow(),
  endsAt: timestamp("ends_at", { withTimezone: true }).default(sql`NULL`),
  isPermanent: boolean("is_permanent").notNull().default(false),
  isTakeABreak: boolean("is_take_a_break").notNull().default(false),
  reason: text("reason").default(sql`NULL`),
  liftedAt: timestamp("lifted_at", { withTimezone: true }).default(sql`NULL`),
  liftedByAdminId: integer("lifted_by_admin_id").default(sql`NULL`),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
});
