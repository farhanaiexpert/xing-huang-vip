...
**Locked glossary precedence (sportsbook):** curated/static terms MUST win over DeepL. Two guarantees: (1) startup merge is `dict = { ...cached, ...STATIC }` (STATIC last) so cached DeepL never overrides curated; (2) runtime DeepL enrichment only fetches strings where `translateString()` returns null, so anything already in the static dict is never sent to DeepL and never overwritten. zh.ts is structured `export const zh = { ...base, ...locked }` so the `locked` Phase-1 glossary overrides any older differing `base` entry without hunting each line. When you change locked terms, bump `CACHE_KEY`.
