...
export function LeagueSection({ league }: { league: League }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-[#253241] bg-[#121821]">
      ...
      {/* League header — click anywhere to collapse/expand */}
      <div
        role="button"
        tabIndex={0}
        onClick={() => setExpanded(e => !e)}
        onKeyDown={e => (e.key === 'Enter' || e.key === ' ') && setExpanded(x => !x)}
        className="flex items-center justify-between px-3.5 py-2.5 bg-[#0F1620] border-b border-[#253241] cursor-pointer select-none hover:bg-[#131B27] transition-colors duration-150 group"
      >
        <div className="flex items-center gap-2 min-w-0 flex-1">
          {/* Collapse chevron */}
          <ChevronDown
            className={`h-3.5 w-3.5 shrink-0 text-[#94A3B8]/50 group-hover:text-[#94A3B8] transition-all duration-200 ${expanded ? 'rotate-0' : '-rotate-90'}`}
          />
      ...
      {/* Collapsible body — smooth grid-row animation */}
      <div
