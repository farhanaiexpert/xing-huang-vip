...
export function PromoPopup() {
  ...
  function close() {
    setClosing(true);
    setTimeout(() => { setVisible(false); setClosing(false); }, 360);
  }
  ...
  return (
    ...
    >
      <style>{`
        ...
        @keyframes pMInMob { from{opacity:0;transform:translateY(40px)} to{opacity:1;transform:translateY(0)} }
        @keyframes pMOutMob{ from{opacity:1;transform:translateY(0)} to{opacity:0;transform:translateY(40px)} }
        @keyframes pMIn    { from{opacity:0;transform:scale(.88) translateY(20px)} to{opacity:1;transform:scale(1) translateY(0)} }
        @keyframes pMOut   { from{opacity:1;transform:scale(1) translateY(0)} to{opacity:0;transform:scale(.94) translateY(12px)} }
        @keyframes pFloat      { from{transform:translateY(0)} to{transform:translateY(-14px)} }
        @keyframes pPillPulse  { 0%,100%{opacity:1} 50%{opacity:.72} }
        @keyframes pCTAPulse   { 0%,100%{box-shadow:0 0 26px rgba(0,223,169,.4),0 4px 14px rgba(0,0,0,.5)} 50%{box-shadow:0 0 42px rgba(0,223,169,.65),0 4px 18px rgba(0,0,0,.5)} }
      ...
      {/* ── MODAL ── */}
      <div
        className="relative w-full sm:max-w-[860px] overflow-hidden shadow-[0_-20px_60px_rgba(0,0,0,0.8)] sm:shadow-[0_40px_100px_rgba(0,0,0,0.95)]"
        style={{
          ...
          maxHeight: '96dvh',
          overflowY: 'auto',
          // Desktop overrides via media query equivalent — handled by Tailwind below
          animation: closing
            ? 'pMOutMob .36s cubic-bezier(.4,0,1,1) forwards'
            : 'pMInMob .4s cubic-bezier(.16,1,.3,1) forwards',
        }}
        onClick={e => e.stopPropagation()}
      >
        {/* Desktop rounded corners override */}
        <style>{`
          @media (min-width:640px) {
            .promo-modal {
              border-radius: 16px !important;
              animation-name: ${closing ? 'pMOut' : 'pMIn'} !important;
            }
          }
        `}</style>

        {/* Top accent */}
        <div className="sticky top-0 inset-x-0 h-[2px] bg-gradient-to-r from-[#00DFA9] via-[#38BDF8] to-[#FACC15] z-10 shrink-0" />
        ...
        <button
          onClick={close}
          className="absolute top-3 right-3 z-20 w-8 h-8 rounded-full flex items-center justify-center text-[#64748B] hover:text-[#F8FAFC] hover:bg-[#1E2A38] transition-all duration-150"
        >
          <X className="w-4 h-4" />
        </button>
        ...
        <div className="flex flex-col sm:flex-row sm:items-stretch">
          ...
          {/* ── IMAGE SECTION ── */}
          {/* Mobile: full-width banner strip, 180px tall */}
          {/* Desktop: left column, 40% wide */}
          <div
            className="relative w-full shrink-0 overflow-hidden sm:w-[40%]"
            style={{ background: DOT_BG, height: '180px', minHeight: '180px' }}
            onMouseEnter={() => { setHovered(true); setShowAlt(true); }}
            onMouseLeave={() => { setHovered(false); setShowAlt(false); }}
          >
            {/* On desktop let it fill naturally */}
            <style>{`
              @media (min-width:640px) {
                .promo-img-panel { height: auto !important; min-height: 280px !important; }
              }
            `}</style>

            <img src={IMG_ORIGINAL} alt="CupBett Ambassador" style={{
              position: 'absolute', inset: 0, width: '100%', height: '100%',
              objectFit: 'cover', objectPosition: 'top center',
              transition: 'opacity 1.4s cubic-bezier(0.4,0,0.2,1)',
              opacity: altVisible ? 0 : 1,
            }} />
            <img src={IMG_ALT} alt="CupBett Ambassador" style={{
              position: 'absolute', inset: 0, width: '100%', height: '100%',
              objectFit: 'cover', objectPosition: 'top center',
              transition: 'opacity 1.4s cubic-bezier(0.4,0,0.2,1)',
            ...
            <div className="absolute inset-x-0 bottom-0 h-16 pointer-events-none sm:hidden"
              style={{ background: 'linear-gradient(to top,#0A0F16,transparent)' }} />
            <div className="absolute inset-y-0 right-0 w-12 pointer-events-none hidden sm:block"
              style={{ background: 'linear-gradient(to right,transparent,#0A0F16)' }} />

            {/* LIVE badge */}
            <div className="absolute top-2.5 left-2.5 flex items-center gap-1.5 rounded-full px-2.5 py-1 border border-[#00DFA9]/30 pointer-events-none"
            ...
            {/* Stars — bottom-left, only when panel is tall enough */}
            <div className="absolute bottom-2.5 left-2.5 hidden sm:flex items-center gap-1 rounded-full px-2.5 py-1 border border-[#FACC15]/20 pointer-events-none"
              style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)' }}>
          ...
          {/* ── CONTENT ── */}
          <div className="relative flex flex-col flex-1 px-4 pt-4 pb-5 sm:px-7 sm:py-7">

            {/* Bonus pill */}
            <div className="flex items-center mb-3 pr-8 sm:pr-0">
              <div className="flex items-center gap-1.5 rounded-full px-2.5 py-1.5 border border-[#FACC15]/30 min-w-0"
            ...
            {/* Headline */}
            <h2 className="text-[17px] sm:text-[24px] font-black text-[#F8FAFC] leading-[1.2] mb-2">
              Connect Your Wallet &amp;{' '}
            ...
            {/* Social proof */}
            <div className="flex items-center gap-2 mb-3 sm:mb-5 p-2 sm:p-2.5 rounded-xl border border-[#1E2A38]"
              style={{ background: 'rgba(18,24,32,0.7)' }}>
              <div className="flex -space-x-2 shrink-0">
                {AVATARS.map((src, i) => (
                  <img key={i} src={src} alt="Player"
                    className="w-6 h-6 sm:w-7 sm:h-7 rounded-full border-2 border-[#0A0F16] object-cover"
                    style={{ zIndex: AVATARS.length - i }}
                    onError={e => { (e.currentTarget as HTMLImageElement).style.display = 'none'; }} />
                ))}
              </div>
              <div className="min-w-0">
                <p className="text-[11px] sm:text-[12px] font-semibold text-[#F8FAFC] truncate">
                  <span className="text-[#00DFA9]">+127 players</span> joined in the last hour
                </p>
                <p className="text-[9px] sm:text-[10px] text-[#94A3B8]">50,000+ active worldwide right now</p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2 mb-3 sm:mb-5">
              <button onClick={handleConnect}
                className="relative w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl font-bold text-[13px] sm:text-[14px] text-[#071210] overflow-hidden transition-transform duration-150 hover:scale-[1.02] active:scale-[0.97]"
              ...
              <button onClick={close}
                className="w-full py-2.5 rounded-xl font-medium text-[12px] text-[#64748B] hover:text-[#94A3B8] border border-[#1E2A38] hover:border-[#253241] hover:bg-[#121821] transition-all duration-150">
                Maybe Later
              </button>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 pt-3 border-t border-[#1E2A38]">
              {([
                ...
                { icon: Shield,     label: 'Provably', sub: 'Fair & Secure',      color: '#FACC15' },
                ...
                <div key={label} className="flex flex-col items-center gap-1 text-center">
                  ...
                  <span className="text-[8px] sm:text-[10px] text-[#64748B] leading-tight">{sub}</span>
                </div>
              ))}
            </div>
          </div>
