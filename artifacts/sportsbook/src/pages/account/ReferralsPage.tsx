...
export function ReferralsPage() {
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-5">
            ...
            <div className="flex gap-3 sm:gap-4">
              <div className="text-center">
                <p className="text-[24px] sm:text-[28px] font-black text-[#00DFA9] leading-none tabular-nums"
                  style={{ textShadow: '0 0 24px rgba(0,223,169,0.35)' }}>
                  <AnimatedNumber value={ref.totalEarned} decimals={2} />
                </p>
                <p className="text-[9px] font-bold text-[#64748B] uppercase tracking-widest mt-0.5">Total Earned</p>
                <p className="text-[9px] text-[#00DFA9]/60">USDT</p>
              </div>
              <div className="w-px bg-white/[0.06]" />
              <div className="text-center">
                <p className="text-[24px] sm:text-[28px] font-black text-[#38BDF8] leading-none tabular-nums"
                  style={{ textShadow: '0 0 24px rgba(56,189,248,0.35)' }}>
                  <AnimatedNumber value={ref.referrals.length} />
                </p>
                <p className="text-[9px] font-bold text-[#64748B] uppercase tracking-widest mt-0.5">Referrals</p>
                <p className="text-[9px] text-[#38BDF8]/60">total</p>
              </div>
              <div className="w-px bg-white/[0.06]" />
              <div className="text-center">
                <p className="text-[24px] sm:text-[28px] font-black text-[#FACC15] leading-none tabular-nums"
                  style={{ textShadow: '0 0 24px rgba(250,204,21,0.35)' }}>
                  <AnimatedNumber value={ref.pendingEarned} decimals={2} />
                </p>
                <p className="text-[9px] font-bold text-[#64748B] uppercase tracking-widest mt-0.5">Pending</p>
                <p className="text-[9px] text-[#FACC15]/60">USDT</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {ref.pendingEarned > 0 && (
        <div className="relative overflow-hidden rounded-2xl p-4 flex items-center justify-between gap-4"
          ...
          <div className="flex items-center gap-3 relative">
            <div className="relative w-10 h-10 flex items-center justify-center rounded-xl shrink-0"
              ...
              <Gift className="w-4.5 h-4.5 text-[#FACC15]" />
            ...
            <div>
              <p className="text-[14px] font-black text-[#F8FAFC]">
                <span className="text-[#FACC15]">{ref.pendingEarned.toFixed(2)} USDT</span> ready to claim
              </p>
              <p className="text-[10px] text-[#94A3B8]">Commission from {ref.commissions.filter(c => c.status === 'pending').length} settled bet{ref.commissions.filter(c => c.status === 'pending').length !== 1 ? 's' : ''}</p>
            </div>
          </div>
          <button
            onClick={() => void handleClaim()}
            disabled={claiming}
            className="relative shrink-0 px-5 py-2.5 rounded-xl text-[12px] font-black text-[#0B0F14] cursor-pointer transition-all hover:scale-[1.02] disabled:opacity-60 disabled:cursor-not-allowed"
          ...
          {TIERS.map(({ level, pct, color, glow, bg, border, label, desc, icon: Icon }) => (
            <div key={level}
              ...
              <div className="relative p-4">
                ...
                <div className="mb-2">
                  <span className="text-[32px] sm:text-[38px] font-black leading-none" style={{ color, textShadow: `0 0 20px ${glow}` }}>{pct}</span>
                  <span className="text-[18px] font-black" style={{ color }}>%</span>
                </div>
                <p className="text-[11px] font-bold text-[#F8FAFC] leading-tight mb-1">{label}</p>
                <p className="text-[9px] text-[#64748B] leading-relaxed hidden sm:block">{desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
        {[
          ...
          <div key={label} className="relative rounded-2xl p-4 border border-[#1E2A38] bg-[#0C1520] overflow-hidden">
            <div className="absolute top-0 left-0 right-0 h-[1.5px]"
              style={{ background: `linear-gradient(90deg, ${color}70, transparent)` }} />
            <div className="flex items-center gap-2 mb-2">
              <Icon className="w-3 h-3" style={{ color, opacity: 0.7 }} />
              <p className="text-[9px] font-bold text-[#64748B] uppercase tracking-wide">{label}</p>
            </div>
            <p className="text-[20px] font-black leading-none" style={{ color }}>
              <AnimatedNumber value={value} decimals={decimals} />
              {suffix && <span className="text-[10px] font-bold ml-1 opacity-60">{suffix}</span>}
            </p>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-2xl p-5 border border-white/[0.06]"
        ...
        <p className="text-[10px] font-bold text-[#64748B] uppercase tracking-widest mb-4">How It Works</p>
          ...
          {[
            ...
            <div key={step} className="flex gap-3">
              <div className="flex flex-col items-center gap-2 shrink-0">
                ...
                <div className="text-[9px] font-black text-[#253241]">{step}</div>
              </div>
              <div className="pt-1">
                <p className="text-[12px] font-bold text-[#F8FAFC] mb-1">{title}</p>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <div className="flex gap-1 p-1 rounded-xl bg-[#0C1520] border border-white/[0.06]">
            {(['commissions', 'referrals'] as const).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={cn(
                  'px-4 py-2 rounded-lg text-[11px] font-bold capitalize transition-all cursor-pointer',
                  activeTab === tab
        ...
        {activeTab === 'commissions' && (
          ref.commissions.length === 0 ? (
            ...
            <div className="rounded-2xl border border-white/[0.07] bg-[#0C1520] overflow-hidden">
              {/* Header row */}
              <div className="grid grid-cols-[auto_1fr_auto_auto] gap-3 px-4 py-2.5 border-b border-white/[0.05]"
                ...
                <p className="text-[9px] font-bold text-[#475569] uppercase tracking-wide text-right">Amount</p>
                <p className="text-[9px] font-bold text-[#475569] uppercase tracking-wide text-right">Status</p>
              </div>
              {ref.commissions.map((c, i) => (
                <div key={c.id} className={cn(
                  'grid grid-cols-[auto_1fr_auto_auto] gap-3 items-center px-4 py-3.5 transition-colors hover:bg-white/[0.02]',
                  i > 0 && 'border-t border-white/[0.04]'
                )}>
                  <LevelBadge level={c.level as 1 | 2 | 3} />
                  <div className="min-w-0">
                    <p className="text-[12px] font-semibold text-[#F8FAFC] truncate">{c.referredAddress || '—'}</p>
                    <p className="text-[10px] text-[#64748B]">{fmtDate(c.date)}</p>
                  </div>
                  <p className="text-[13px] font-bold text-[#00DFA9] text-right">+{c.earned.toFixed(2)}<span className="text-[9px] ml-0.5 opacity-60">USDT</span></p>
            ...
            <div className="rounded-2xl border border-white/[0.07] bg-[#0C1520] overflow-hidden">
              ...
              {ref.referrals.map((r, i) => (
                <div key={r.id} className={cn(
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[12px] font-semibold text-[#F8FAFC] truncate">{r.address}</p>
