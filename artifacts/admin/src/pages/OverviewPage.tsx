  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        />
        <StatCard
          icon={Receipt} label="Total Bets"
          value={s.bets.total.toLocaleString()}
          sub={`${s.bets.open} open`}
          color="bg-[#00DFA9]/10 text-[#00DFA9]"
        />
        <StatCard
          icon={TrendingUp} label="Bet Volume"
          value={`$${fmt(s.bets.volume)} USDT`}
          color="bg-[#FACC15]/10 text-[#FACC15]"
        />
        <StatCard
          icon={Clock} label="Pending Deposits"
          value={s.transactions.pendingDeposits}
          color="bg-[#FACC15]/10 text-[#FACC15]"
        />
        <StatCard
          icon={CreditCard} label="Pending Withdrawals"
          value={s.transactions.pendingWithdrawals}
          color="bg-red-500/10 text-red-400"
        />
        <StatCard
          icon={Wallet} label="Total Wallet Balance"
          value={`$${fmt(s.platform.totalWalletBalance)} USDT`}
          sub={`Commissions paid: $${fmt(s.platform.totalCommissionsPaid)}`}
          color="bg-[#00DFA9]/10 text-[#00DFA9]"
        />
      </div>
    </div>
