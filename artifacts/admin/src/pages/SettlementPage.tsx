  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 text-center">
      ...
      <div className="bg-[#111827] border border-white/8 rounded-xl p-4 w-full max-w-lg">
        ...
        <p className="text-3xl font-bold text-[#00DFA9] font-mono">${fmt(result.totalPaidOut)} <span className="text-base text-[#475569]">USDT</span></p>
      </div>
...
function EventPanel({
  ...
  if (result) {
    return <SuccessCard result={result} eventName={detail.eventName} onReset={onBack} />;
  }

  // Quick-set helpers: mark all selections in a market as a given result, or reset
  function applyAll(result: OutcomeResult) {
    const next: Record<string, OutcomeResult> = {};
    for (const m of detail.markets) {
      next[outcomeKey(m.market_type, m.selection)] = result;
    }
    setOutcomes(next);
  }
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <button onClick={onBack}
            className="flex items-center gap-1.5 text-xs text-[#475569] hover:text-white transition-colors mb-2">
...
export default function SettlementPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5 pb-10">
      {/* Page header */}
      <div>
        <h1 className="text-2xl font-bold text-white tracking-tight">Batch Settlement</h1>
        <p className="text-sm text-[#475569] mt-0.5">
          Select an event, assign results to each market, and settle all bets in one action.
        </p>
      </div>

      {/* How it works — shown only when no event is selected */}
      {!selectedEventId && (
      ...
      <div className="flex gap-4">
        ...
        <div className={cn(
          ...
          selectedEventId ? "w-72 shrink-0" : "flex-1"
        )}>
